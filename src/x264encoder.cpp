...
void x264Encoder::initialize(const unsigned int w, const unsigned int h,
                             const bool isBGR, const bool hasAlpha,
                             const int threads) {
...
void x264Encoder::encodeFrame(const char *rgb_buffer) {
  ...
  long int interval =
      (long)std::chrono::duration_cast<std::chrono::milliseconds>(
          ...
          .count() -
      time_base_;
  time_base_ = (long)std::chrono::duration_cast<std::chrono::milliseconds>(
                   p.time_since_epoch())
                   .count();
  frame_size = x264_encoder_encode(encoder_, &nals, &i_nals, &picture_in_,
                                   &picture_out_);
  if (!first_) acc_(interval);
...
void x264convertmp4(std::string input, std::string output, bool deleteinput, x264Encoder & enc)
{
  ...
  if (std::system(command.c_str()) == 0) {
    ...
    if (deleteinput && std::system((std::string("rm ") + input).c_str()) == 0)
