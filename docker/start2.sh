...
cd data
export data_path="/home/lando/pelars/client/data"
cd $working_path
...
docker run -t -i --privileged --net=host -v /dev/snd:/dev/snd --device=/dev/video0:/dev/video0   -v /dev/bus/usb:/dev/bus/usb -v /etc/udev/rules.d:/etc/udev/rules.d --device=/dev/nvidia0:/dev/nvidia0 --device=/dev/nvidiactl:/dev/nvidiactl --device=/dev/nvidia-uvm:/dev/nvidia-uvm -v $data_path:/data  -v ~/NVIDIA_CUDA-7.5_Samples/bin/x86_64/linux/release:/nvidia -v $data_path:/data -e DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix:rw  pelars_compile2 /bin/bash  #/pelars/bin/sensors/sensor_manager $VARS
